...
class GeneralWorker_Read(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        qs = (
            GeneralWorker.objects.filter(company=request_user.company)
            .select_related(
...
def get_obj_worker(obj):
    ...
    return {
        ...
        'doc_url': obj.doc.url if obj.doc and obj.doc.name and obj.doc.storage.exists(obj.doc.name) else '',
        'doc_name': obj.doc.name if obj.doc and obj.doc.name and obj.doc.storage.exists(obj.doc.name) else '',
